...
from distributed.protocol.scipy import serialize_scipy_sparse_dok
...
def select_option(prompt, options):
    while True:  # 제대로된 입력값이 들어올 때까지 반복
        ...
        for idx, option in enumerate(options, start=1):  # 시퀀스 자료형, 시작값
            print(f"[{idx}] {option}")
        sel = input("선택하세요 : ")
        if sel in map(str, range(1, len(options) + 1)):  # 메뉴가 2개이상 존재한다면
            return sel
...
def make_ipad(screen, color, memory, network, name):
    ...
    # 시리얼 넘버 만들기
    serial_screen = "11" if screen == "1" else "13"  ### 삼항연산자!
    serial_network = "W" if screen == "1" else "C"
