...
# 내부 변수를 외부에서 접근하고자할 때 지정된 게터와 세터 메소드를 통해서만 접근하도록 하는 것
# private(__) : 같은 클래스 내에서만 접근 가능
# protected(_) : 같은 클래스 내이거나 상속관계가 존재하면 접근 가능
# public(변수에 아무것도 안붙임) : 아무데서나 접근 가능

class TV:
    cnt = 0 # 클래스 멤버
    def __init__(self, name, isOn, channel, volume):
        self.__name = name  # private은 클래스 내부에서만 접근 가능
        self.__isOn = isOn
...
lg_tv.__name = "SAMSUNG" # 에러는 안나지만 변경은 안됨 (원래는 에러 나야됨)
lg_tv.__isOn = False # 에러는 안나지만 변경은 안됨 (원래는 에러도 나야됨)
