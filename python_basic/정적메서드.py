...
# 정적 메서드 : 클래스와 관련이 없음, 객체와 무관한 독립적인 동작 수행
# 클래스 메서드 : 클래스와 관련 있음, 클래스 변수를 사용하기 위함
class Car:
    is_instance_cnt = 0  # 클래스 변수(멤버), 클래스 소속, 객체로 만들어 지지 않음

    def __init__(self, size, model):
        self.size = size  # 이렇게 self 붙은건 인스턴스 변수
        self.model = model
        self.speed = 0
    ...
    @staticmethod  # 클래스의 영향을 받지 않고 별도로 움직이는 함수처럼 사용해야될 땐 정적 메서드를 사용함
    def check_type(code):
        if code <= 10: print("전기차 입니다.")
        elif code <= 20: print("가솔린차 입니다.")
        elif code <= 30: print("디젤차 입니다.")
        else: print("분류 코드가 없습니다.")
        # Car.is_instance_cnt 이런식으로 클래스 변수 사용 가능
