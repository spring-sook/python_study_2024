...
# 함수는 매개변수(parameter)를 가질 수 있고, 반환값을 가질 수 있음
# 식별자는 스네이크 표기법으로 작성하고, 식별자 뒤에 ()소괄호가 있음
# 일반적으로 매개변수와 함수의 호출하는 인자(argument)의 갯수가 일치해야 함
# def 키워드를 사용해서 정의
...
# name_card("장원영", "서울시 강남구 삼성동", "010-8765-4321", "센터")
# name_card("가을", "서울시 강남구 신사동", "010-4321-5678", "싱어")

# 은행 계좌 개설하기
def open_account(name):  # 매개변수도 있고, 반환값은 존재하지 않는 함수 선언
...
lang = "Python", "Java", "C", "C++", "React", "Kotlin", "Swift"  # 이거랑 형태가 같은거임
# print(lang)
#
...
# 파이썬의 경우는 함수 스코프 기반의 언어
# 외부에서 선언한 변수의 값을 함수 내에서 변경하기 위해서는 global 키워드를 사용해야 함

knife = 10  # 게임에 사용할 칼을 10자루 준비
def game(player):
    global knife
...
weight = round(std_weight(h, g), 2)  # 소수점 3번째 자리에서 반올림해서 소수점 2자리로 만듦
gender_str = "", "남성", "여성"
